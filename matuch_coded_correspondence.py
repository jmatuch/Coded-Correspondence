...
def decoder(message,offset):
    ...
    for n in message:
        if n.isalpha():
            index = 97 + ((ord(n) - 97 + offset)%26)
            decoded = decoded + chr(index)
        else:
            decoded = decoded + n
    return decoded
print(decoder(string,10))
...
def encoder(message,offset):
    ...
    for n in message:
        if n.isalpha():
            ...
            code = code + n
    return code
print(encoder("hello! my name is john smith.",5))
print(decoder("czggj! ht ivhz dn ejci nhdoc.",5))
...
string = "dfc aruw fsti gr vjtwhr wznj? vmph otis! cbx swv jipreneo uhllj kpi rahjib eg fjdkwkedhmp!"
string2 = "friends"
def newDecoder(message,keyword):
    decoded = ''
    lengthKeyword = len(keyword)
    letterCount = 0
    messageCount = 0
    for n in message:
        if n.isalpha():
            keywordIndex = messageCount%lengthKeyword
            index = 97 + ((ord(n) - 97 - (ord(keyword[keywordIndex])-97))%26)
            decoded = decoded + chr(index)
            messageCount += 1
        else:
            decoded = decoded + n
    return decoded
print(newDecoder(string,string2))
...
string = "i can't believe how much i've learned about cryptography! it's really amazing!"
string2 = "shuffle"
def newEncoder(message,keyword):
    encoded = ''
    lengthKeyword = len(keyword)
    letterCount = 0
    messageCount = 0
    for n in message:
        if n.isalpha():
            keywordIndex = messageCount%lengthKeyword
            index = 97 + ((ord(n) - 97 + (ord(keyword[keywordIndex])-97))%26)
            encoded = encoded + chr(index)
            messageCount += 1
        else:
            encoded = encoded + n
    return encoded
code = newEncoder(string,string2)
print(code)
print(newDecoder(code,string2))
